import { GameContext, GameEvent, GameGuard, Player, PlayerColors } from "../types"
import { emptyCellRow, gridCellLeft, winningRowPosition } from "../func/game"


export const canJoinGuard: GameGuard<"join"> = (context, event) => {
	return context.players.length < 2 && context.players.find((p) => p.id === event.playerId) === undefined
}

export const canLeaveGuard: GameGuard<"leave"> = (context, event) => {
	return context.players.find((p) => p.id === event.playerId) !== undefined
}

export const canPickColorGuard: GameGuard<"pickColor"> = (context, event) => {
	return [PlayerColors.RED, PlayerColors.YELLOW].includes(event.color) &&
		context.players.find(p => p.id === event.playerId) !== undefined &&
	  context.players.find((p) => p.color === event.color) === undefined
}

export const canStartGuard: GameGuard<"start"> = (context) => {
	return context.players.filter(p => p.color).length === 2
}

export const canDropTokenGuard: GameGuard<"dropToken"> = (context, event) => {
	return event.x >= 0 && event.x < context.grid[0].length &&
		context.currentPlayer === event.playerId &&
		emptyCellRow(context.grid, event.x) >= 0
}

export const isWinningMoveGuard: GameGuard<"dropToken"> = (context, event) => {
	const color = context.players.filter(p => p.id === event.playerId)[0].color
	return canDropTokenGuard(context, event) && color &&
		winningRowPosition(context.grid, color, event.x, context.rowLength).length > 0
}

export const isDrawMoveGuard: GameGuard<"dropToken"> = (context, event) => {
	return gridCellLeft(context.grid) <= 1
	/*return winningRowPosition(context.grid, 
			context.players.filter(p => p.id === event.playerId)[0].id, 
			event.x, context.rowLength).length > 0 &&*/
}
